...
@section('crumb')
    <x-bread-crumb :breadcrumbs="[
        ['text'=> TranslationHelper::translate('port' , 'admin') ,'link'=>route('admin.port.index')],
        ['text'=> TranslationHelper::translate( Str::replace(' ' , '_', Str::lower(getLastKeyRoute(request()->route()->getName()))) , 'admin')]
        ]" :button="['text'=> TranslationHelper::translate('go_to_ports' , 'admin') ,'link'=>route('admin.port.index')]"
        :permission="auth('admin')->user()->can('ports.read')">
    </x-bread-crumb>
@endsection
                            ...
                            <div class="fv-row mb-10">
                                <div class="row">
                                    ...
                                    <div class="col-{{ count(Config('language')) > 1  ? 6 : 12 ;}}">
                                        ...
                                        <!--end::Label-->
                                        <!--begin::Input-->
                                        <input type="text" class="form-control form-control-solid" required value="{{ old('name.'.$key) ?? $port->getTranslation('name',$key)}}" placeholder="{{ $lang }}" name="name[{{ $key}}]" />
                                        @error('name.'.$key)
