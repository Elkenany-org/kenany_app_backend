@adminCan('products.edit')
<a href="{{ route('admin.product.edit', $product->id) }}" class="btn btn-success btn-sm mr-2" data-toggle="tooltip" data-placement="top"
   title="Show">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title text-danger" id="exampleModalToggleLabel">
                    {{ TranslationHelper::translate('delete' , 'admin') }} {{ $product->title }}</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form action="{{ route('admin.product.destroy',$product->id) }}" method="POST" class="btn_delete">
                    @csrf
                    @method('DELETE')
                        {{ TranslationHelper::translate('are_you_sure_you_want_to_delete' , 'admin') }} {{ $product->title }} 
                    <br/>
                    <hr>
